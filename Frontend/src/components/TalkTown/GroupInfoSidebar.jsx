...
const GroupInfoSidebar = ({ chatId, onClose, onDeleteSuccess }) => {
    ...
    return (
        <div className="w-full lg:w-80 h-full bg-zinc-950 border-l border-zinc-800 flex flex-col absolute right-0 top-0 z-30 shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div className="p-8 flex flex-col items-center gap-4 text-center border-b border-zinc-900">
                    ...
                    {isEditingName ? (
                        <div className="flex items-center gap-2">
                            <input
                                className="bg-black border border-zinc-700 rounded px-2 py-1 text-sm outline-none"
                                value={newName}
                                onChange={(e) => setNewName(e.target.value)}
                            />
                            <button onClick={handleRename} className="text-blue-500 text-xs font-bold">Save</button>
                        </div>
                    ) : (
                        <div className="flex items-center gap-2 group">
                            <h3 className="text-lg font-bold text-white">{chatDetails.name}</h3>
                            <Edit2 size={14} className="text-zinc-500 cursor-pointer opacity-0 group-hover:opacity-100" onClick={() => setIsEditingName(true)} />
                        </div>
                    )}
                    <p className="text-xs text-zinc-500 font-medium italic">Group · {chatDetails.members.length} members</p>
                </div>

                {/* Members List Section */}
             
                <div className="p-4">
                    <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-4">Members</h4>
                            ...
                            <div key={member._id} className="flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        {member._id === chatDetails.createdBy && <span className="text-[10px] text-blue-500 font-bold">Admin</span>}
                                    </div>
                                </div>
                                {isAdmin && member._id !== currentUser?._id && (
                                    <button onClick={() => handleRemoveMember(member._id)} className="text-zinc-600 hover:text-red-500 transition opacity-0 group-hover:opacity-100">
                                        <UserMinus size={16} />
                                    </button>
                                )}
                            </div>
            ...
            <div className="p-4 border-t border-zinc-900 bg-zinc-900/20">
                {isAdmin ? (
                    <button
                            ...
                            if (window.confirm("Delete this group permanently?")) {
                                try {
                                    ...
                                    if (res.success) {
                                        ...
                                        onDeleteSuccess(); // ChatWindow se context update
                                    }
                                } catch (err) {
                        ...
                        className="w-full flex items-center justify-center gap-2 text-red-500 hover:bg-red-500/10 py-3 rounded-xl transition text-sm font-bold border border-red-500/20"
                    >
