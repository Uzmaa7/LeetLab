...
import { getActiveContestsService, getCreatedContestsService } from "../services/contest.services.js";
import { getAllCollectionsService } from "../services/collection.service.js";
import { createContestService } from "../services/contest.services";
import { joinContestService } from "../services/contestParticipant.service.js";
...
function ContestsPage() {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // console.log("aaaaaaaaaaa", collectionsData)

                setActiveContests(contestData?.data?.contests || []);
                setCollections(collectionsData?.Collections);
                setCollectionOptions(collectionsData?.Collections.map((c) => ({
    ...
    //                 <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-900 border border-zinc-800 text-[10px] font-black uppercase tracking-widest text-orange-500 mb-2">
    ...
    return (
        // BG Color 
        <div className="min-h-screen bg-[#050505] px-4 pt-20 md:pt-24 pb-20 relative font-sans">
            ...
            <div className="relative z-10 max-w-6xl mx-auto space-y-16">
                ...
                {/* HERO SECTION */}
                <section className="text-center space-y-4">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-zinc-900/50 border border-zinc-800 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400 mb-2">
                ...
                <section className="grid md:grid-cols-2 gap-6">
                    ...
                    {/* JOIN CONTEST CARD */}
                    <div ref={joinFormRef} className="bg-[#0d0d0d] border border-zinc-800/60 rounded-xl p-8 hover:border-zinc-700 transition-all shadow-sm">
                        <div className="flex items-center gap-4 mb-8">
                            <div className="p-3 bg-zinc-900 rounded-lg text-zinc-300 border border-zinc-800">
                                <Zap size={22} />
                            </div>
                            <div>
                                <h2 className="text-lg font-semibold text-white">Join a Contest</h2>
                                <p className="text-xs text-zinc-500">Enter code to participate</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={joinContesthandler} className="space-y-6">
                            <Input
                                label="Contest Code"
                                placeholder="7890-XYZ"
                                value={contestCode}
                                onChange={(e) => setContestCode(e.target.value)}
                                required
                                className="bg-[#050505] border-zinc-800 focus:border-indigo-500 text-zinc-300 h-11"
                            />

                            <Button type='submit' className="w-full h-11 text-sm font-semibold bg-zinc-600 hover:bg-zinc-500 text-zinc-50 transition-all rounded-md shadow-sm active:scale-[0.98]">
                                Enter Arena
                            </Button>
                        </form>
                    </div>
                    ...
                    {/* CREATE CONTEST CARD */}
                    <div ref={createFormRef} className="bg-[#0d0d0d] border border-zinc-800/60 rounded-xl p-8 hover:border-zinc-700 transition-all shadow-sm">
                        <div className="flex items-center gap-4 mb-8">
                            <div className="p-3 bg-zinc-900 rounded-lg text-zinc-300 border border-zinc-800">
                                <Plus size={22} />
                            </div>
                            <div>
                                ...
                                <p className="text-xs text-zinc-500">Setup a new assessment lobby</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={createContestHandler} className="space-y-5">
                            <Input
                                label="Contest Title"
                                placeholder="Internal Backend Assessment"
                                value={contestTitle}
                                onChange={(e) => setContestTitle(e.target.value)}
                                required
                                className="bg-[#050505] border-zinc-800 text-zinc-300 h-11"
                            />

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="flex flex-col gap-2">
                                    <label className="text-xs font-semibold text-zinc-400 ml-1">Collection</label>
                                    <Select
                                        value={contestCollectionId}
                                        onChange={(e) => setContestCollectionId(e.target.value)}
                                        options={collectionOptions}
                                        placeholder="Select..."
                                        className="w-full bg-[#050505] border-zinc-800 text-zinc-300 h-11 px-3 rounded-md focus:border-indigo-500 transition-all outline-none appearance-none"
                                    />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label className="text-xs font-semibold text-zinc-400 ml-1">Visibility</label>
                                    <Select
                                        value={contestVisibility}
                                        onChange={(e) => setContestVisiblity(e.target.value)}
                                        options={VISIBILITY_OPTIONS}
                            ...
                            {selectedCollection && selectedCollection.questionsCount === 0 && (
                                <div className="flex items-start gap-2 p-3 rounded-md bg-red-950/20 border border-red-900/30 text-red-500 text-[11px]">
                                    <AlertCircle size={14} className="shrink-0" />
                                    <p>Selected collection is empty.</p>
                                </div>
                            )}



                            {/* ROW 2: Questions & Duration */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="flex flex-col gap-2">
                                    <label className="text-xs font-semibold text-zinc-400 ml-1">Questions</label>
                                    <Input
                                        type="number"
                                        min={1}
                                        max={selectedCollection ? selectedCollection.questionsCount : 5}
                                        ...
                                        onChange={(e) => setContestQuestionCount(e.target.value)}
                                        disabled={selectedCollection?.questionsCount === 0}
                                        className="w-full bg-[#050505] border-zinc-800 text-zinc-300 h-11 px-4 rounded-md focus:border-indigo-500 transition-all outline-none"
                                    />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label className="text-xs font-semibold text-zinc-400 ml-1">Duration</label>
                                    <Select
                                        value={contestDuration}
                                        onChange={(e) => setContestDuration(e.target.value)}
                                        options={DURATION_OPTIONS}
                            ...
                            <Button
                                type='submit'
                                disabled={selectedCollection?.questionsCount === 0}
                                className={`w-full h-11 text-sm font-semibold transition-all rounded-md shadow-sm active:scale-[0.98] 
                ...
                <section ref={navRef} className="space-y-6">
                    <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                        <LayoutList size={18} className="text-zinc-500" /> Your Dashboard
                    </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[
                            ...
                            {
                                title: 'Created',
                                subtitle: 'Manage your lobbies',
                                path: '/contests/created'
                            },
                            {
                                title: 'Joined',
                                subtitle: 'Track participated events',
                                path: '/contests/joined'
                            }
                        ].map((item, idx) => (
                            <div
                                key={idx}
                                onClick={() => navigate(item.path)}
                                className="group cursor-pointer bg-[#0d0d0d] border border-zinc-800/60 p-6 rounded-xl hover:border-zinc-600 transition-all flex items-center justify-between shadow-sm"
                            >
                                <div>
                ...
                {/* ACTIVE CONTESTS */}
                <section ref={activeRef} className="space-y-6 pt-10 border-t border-zinc-900">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="bg-[#0d0d0d] border border-zinc-800/60 rounded-xl overflow-hidden shadow-2xl shadow-black">
                        {activeContests.length > 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-16 text-zinc-600 space-y-3">
                                ...
                                <p className="text-xs font-medium tracking-wide">NO ACTIVE SESSIONS</p>
                            </div>
                        )}
                    </div>
                </section>
